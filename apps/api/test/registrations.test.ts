...
const CREDS = { email: "registration-owner@example.com", password: "password123" };

// Scaffolder nie potrafił automatycznie zseedować relacji wymaganych: eventId.
...
/** CRUD encji registrations — wygenerowane przez scaffolder. */
describe.skipIf(!url || RELATIONS_TODO)("registrations CRUD (wygenerowane)", () => {
  ...
  beforeEach(async () => {
    await db.execute(sql`TRUNCATE TABLE users, events, registrations CASCADE`);
  ...
  it("wymaga uwierzytelnienia (401 bez cookie)", async () => {
    const res = await app.inject({ method: "GET", url: "/api/v1/registrations" });
  ...
  it("list zawiera utworzony rekord", async () => {
    await create();
    const res = await app.inject({ method: "GET", url: "/api/v1/registrations", cookies: auth() });
  ...
  it("get 404 dla nieistniejącego id", async () => {
    const res = await app.inject({
      method: "GET",
      url: "/api/v1/registrations/00000000-0000-0000-0000-000000000000",
      cookies: auth(),
    });
  ...
  it("delete jest miękkie: po usunięciu get→404 i znika z listy", async () => {
    ...
    expect(
      (
        await app.inject({
          method: "DELETE",
          url: `/api/v1/registrations/${created.id}`,
          cookies: auth(),
        })
      ).statusCode,
    ).toBe(200);
