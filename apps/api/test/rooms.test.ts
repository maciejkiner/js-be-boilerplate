...
const CREDS = { email: "room-owner@example.com", password: "password123" };

// Scaffolder nie potrafił automatycznie zseedować relacji wymaganych: venueId.
...
/** CRUD encji rooms — wygenerowane przez scaffolder. */
describe.skipIf(!url || RELATIONS_TODO)("rooms CRUD (wygenerowane)", () => {
  ...
  beforeEach(async () => {
    await db.execute(sql`TRUNCATE TABLE users, venues, rooms CASCADE`);
  ...
  async function create() {
    // TODO: utwórz rekord `venue` (relacja wymagana) i podstaw jego id.
    const venueId = "";
    return app.inject({
      method: "POST",
      url: "/api/v1/rooms",
  ...
  it("wymaga uwierzytelnienia (401 bez cookie)", async () => {
    const res = await app.inject({ method: "GET", url: "/api/v1/rooms" });
  ...
  it("list zawiera utworzony rekord", async () => {
    await create();
    const res = await app.inject({ method: "GET", url: "/api/v1/rooms", cookies: auth() });
  ...
  it("get 404 dla nieistniejącego id", async () => {
    const res = await app.inject({
      method: "GET",
      url: "/api/v1/rooms/00000000-0000-0000-0000-000000000000",
      cookies: auth(),
    });
  ...
  it("delete jest miękkie: po usunięciu get→404 i znika z listy", async () => {
    ...
    expect(
      (await app.inject({ method: "DELETE", url: `/api/v1/rooms/${created.id}`, cookies: auth() }))
        .statusCode,
    ).toBe(200);
