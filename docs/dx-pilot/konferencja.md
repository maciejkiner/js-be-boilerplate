...
| 2   | **`date` gubi godzinę** — kolumna to `timestamp with timezone`, ale formularz renderuje `<input type="date">`                     | `be-templates.ts:34` vs `design-system/src/fields.tsx:41` (mapowanie `packages/forms-ui/src/field-renderer.tsx:131`)                  | P1, P3 — konferencja bez godzin nie istnieje   |
