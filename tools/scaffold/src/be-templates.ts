...
/** Relation fields filterable by equality plus enums — the API filters (as in the reference module). */
...
export function drizzleSchema(d: EntityDescriptor): string {
  ...
  // A PARTIAL index (`where deleted_at is null`): a soft delete releases the value, so a
  // soft-deleted row does not block reusing the unique field.
...
/** Table ${d.table} — generated by the scaffolder from the \`@repo/schemas\` entity. */
...
/** Data access for ${d.plural} — queries only (soft-delete aware). Generated. */
...
export function service(d: EntityDescriptor): string {
  ...
  // We recognise a uniqueness violation by the index name (generated deterministically), so the 409
  // names the specific fields — both in `detail` (for a human) and in the `errors` extension (for a
  // form, which highlights the control).
...
`
  ...
  const uniqueImport = hasUnique
    ...
    : "";
  // `.catch(rethrowAsConflict)` instead of try/catch: the helper returns `never`, so the result type
  // stays intact (a `let`-based variant would widen it to `any`).
...
${assertFn}${uniqueFn}
/** Business logic for ${d.plural}. Generated. */
...
export function beTest(d: EntityDescriptor): string {
  ...
  // Required relations the scaffolder CANNOT seed automatically: a target entity other than
  // `project` (the only payload it knows) and `user` (we have `userId` from registration). For those
  // we generate a compilable stub plus a TODO, and the whole suite is skipped (RELATIONS_TODO) —
  // rather than pretending to pass or emitting a test that breaks or does not compile.
  ...
  const manualPrereq = manualRelations
    .map(
      (f) =>
        `    // TODO: create a \`${f.relation!.entity}\` record (a required relation) and use its id.\n    const ${f.name} = "";\n`,
    )
    .join("");
  ...
  const todoNote = relationsTodo
    ? `\n// The scaffolder could not seed these required relations automatically: ${manualRelations
        .map((f) => f.name)
        .join(
          ", ",
...
${todoNote}
/** CRUD for the ${d.plural} entity — generated by the scaffolder. */
describe.skipIf(${skipCond})("${d.plural} CRUD (generated)", () => {
...
${bodyEntries}
  ...
  it("delete is soft: after it, get returns 404 and the row leaves the list", async () => {
...
/** CRUD for ${d.plural} under /api/v1/${d.path}. Generated: routes → service → repository; auth required. */
