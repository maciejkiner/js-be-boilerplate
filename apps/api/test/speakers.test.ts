...
const CREDS = { email: "speaker-owner@example.com", password: "password123" };

/** CRUD encji speakers — wygenerowane przez scaffolder. */
describe.skipIf(!url)("speakers CRUD (wygenerowane)", () => {
  ...
  beforeEach(async () => {
    await db.execute(sql`TRUNCATE TABLE users, speakers CASCADE`);
  ...
  it("wymaga uwierzytelnienia (401 bez cookie)", async () => {
    const res = await app.inject({ method: "GET", url: "/api/v1/speakers" });
  ...
  it("list zawiera utworzony rekord", async () => {
    await create();
    const res = await app.inject({ method: "GET", url: "/api/v1/speakers", cookies: auth() });
  ...
  it("get 404 dla nieistniejącego id", async () => {
    const res = await app.inject({
      method: "GET",
      url: "/api/v1/speakers/00000000-0000-0000-0000-000000000000",
      cookies: auth(),
    });
  ...
  it("delete jest miękkie: po usunięciu get→404 i znika z listy", async () => {
    ...
    expect(
      (
        await app.inject({
          method: "DELETE",
          url: `/api/v1/speakers/${created.id}`,
          cookies: auth(),
        })
      ).statusCode,
    ).toBe(200);
